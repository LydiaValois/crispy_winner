...
people = [paul, jane, kate, nick]
names = ['Paul', 'Jane', 'Kate', 'Nick']
vocab = []
for i in people:
    i = i.lower().translate({ord(c): None for c in string.punctuation})
    # i = i.lower().translate(str.maketrans('', '', string.punctuation))
    i_list = i.split()
    i_set = set(i_list)
    i_len = len(i_set)
    vocab.append(i_len)
    reviewers = zip(names, vocab)
print(tuple(reviewers))


# nick = nick.lower().translate({ord(c): None for c in string.punctuation})
# # nick = nick.lower().translate(str.maketrans('', '', string.punctuation))
# nick_list = nick.split()
# # nick_set = set(nick_list)
# print(nick_list)